...
djdw_0 = C*(w - w_star) - 2*w*dot(tau_new, grad(eta_new)) - 2*w*rho_new*div(nu_new)
djdw = C*(w - w_star) - 2*w*dot(tau_new, grad(eta_new)) - 2*w*rho_new*div(nu_new) + K*(w - w_prev)
...
def update_control(u_all, v_all, w_all_k, lr):
    ...
    u_new.assign(u_all[0])
    v_new.assign(v_all[0])
    w.assign(w_all_k[0])

    wt.assign( project( (w - lr*djdw_0), FS) )
...
while(count < dal):
    ...
    Hdf = HDF5File(mesh.mpi_comm(), f'{data_dir}/data_test_count_{count}_w0_{w_0_star}_K_{K}.h5', "w")
    Hdf.write(mesh, "mesh")
    ...
    if(rank == 0):
        np.savez(f"{data_dir}/cost_{w_0_star}_{K}.npz", J = penalty, lr = learning)
    count = count + 1
